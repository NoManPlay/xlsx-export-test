      ...
      function write_num_int(type, fmt, val) {
        ...
        switch (fmt) {
          ...
          default:
            if (fmt.match(/\.[0#?]*$/))
              return (
                write_num_int(type, fmt.slice(0, fmt.lastIndexOf('.')), val) + hashq(fmt.slice(fmt.lastIndexOf('.')))
              )
        }
        throw new Error('unsupported format |' + fmt + '|')
      }
...
*/
  ...
  var __toBuffer = function (bufs) {
    ...
    for (var i = 0; i < bufs[0].length; ++i)
      if (bufs[0][i]) for (var j = 0, L = bufs[0][i].length; j < L; j += w) x.push.apply(x, bufs[0][i].slice(j, j + w))
    return x
  }
  ...
  function parse_FILETIME(blob) {
    ...
    return new Date(((dwHighDateTime / 1e7) * Math.pow(2, 32) + dwLowDateTime / 1e7 - 11644473600) * 1000)
      .toISOString()
      .replace(/\.000/, '')
  }
        ...
        switch (RT) {
          ...
          default:
            if ((R_n || '').indexOf('Begin') > 0) {
              ...
              !pass ||
              (opts.WTF && state[state.length - 1] != 'BrtACBegin' && state[state.length - 1] != 'BrtFRTBegin')
            )
              throw new Error('Unexpected record ' + RT + ' ' + R_n)
        }
      },
        ...
        case 'data' /*case 'Data'*/:
          ...
          if (Rn[1] === '/')
            parse_xlml_data(
              ...
              ss,
              dtag,
              state[state.length - 1][0] == /*"Comment"*/ 'comment' ? comment : cell,
              { c: c, r: r },
              styles,
              cstys[c],
              row,
...
*/
    ...
    while (blob.l < blob.length - 1) {
      ...
      if (R && R.f) {
        ...
        if (R.r === 2 || R.r == 12) {
          var rt = blob.read_shift(2)
          length -= 2
          if (!opts.enc && rt !== RecordType && (((rt & 0xff) << 8) | (rt >> 8)) !== RecordType)
  ...
  function parse_xlscfb(cfb, options) {
    ...
    if (options.bookProps && !options.bookSheets) WorkbookP = {}
      ...
      /* Quattro Pro 7-8 */ else if ((_data = CFB.find(cfb, 'PerfectOffice_MAIN')) && _data.content)
        WorkbookP = WK_.to_workbook(_data.content, ((options.type = T), options))
      /* Quattro Pro 9 */ else if ((_data = CFB.find(cfb, 'NativeContent_MAIN')) && _data.content)
        WorkbookP = WK_.to_workbook(_data.content, ((options.type = T), options))
  ...
  function write_biff8_global(wb, bufs, opts) {
    ...
    for (j = 0; j < wb.SheetNames.length; ++j) {
      ...
      write_biff_rec(
        B,
        'BoundSheet8',
        write_BoundSheet8({ pos: start, hs: _sheet.Hidden || 0, dt: 0, name: wb.SheetNames[j] }, opts)
      )
      start += bufs[j].length
    }
    /* 1*BoundSheet8 */
  ...
  function write_zip_type(wb, opts) {
    ...
    var out = z.FullPaths
      ? CFB.write(z, { fileType: 'zip', type: { nodebuffer: 'buffer', string: 'binary' }[oopts.type] || oopts.type })
      : z.generate(oopts)
    /*jshint -W083 */
  ...
  ;(function (utils) {
    ...
    utils.book_append_sheet = function (wb, ws, name) {
      if (!name)
        for (var i = 1; i <= 0xffff; ++i, name = undefined) if (wb.SheetNames.indexOf((name = 'Sheet' + i)) == -1) break
